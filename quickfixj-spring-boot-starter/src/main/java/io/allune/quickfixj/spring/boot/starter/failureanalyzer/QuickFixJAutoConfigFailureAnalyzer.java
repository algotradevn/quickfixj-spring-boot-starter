...
/**
 * The failure analyzer is responsible to provide readable information of
 * exception which occur on startup. All exception based on the
 * {@link QuickFixJBaseException} are handled here.
 *
...
extends AbstractFailureAnalyzer<QuickFixJBaseException> {
	...
	protected FailureAnalysis analyze(final Throwable rootFailure,
			final QuickFixJBaseException cause) {
		String descriptionMessage = cause.getMessage();
		String actionMessage = cause.getMessage();
		final String rootCauseMessage = getRootCauseMessage(cause);
	...
	private String getRootCauseMessage(final QuickFixJBaseException cause) {
